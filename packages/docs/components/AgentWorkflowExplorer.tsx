...
import { AGENT_FINANCE, AGENT_GMAIL, AGENT_PDF, AGENT_TRELLO } from './TabSelector'
...
//Map the agents keys to the github URLs of the repository
type endpointsType = { [key: string]: string }
const exampleEndPoints: endpointsType = {
...
export const AgentWorkflowExplorer: React.FC<AgentWorkflowExplorer> = ({ agent, initialData }) => {
  ...
  const MotiaNav = () => {
    const href = `${GITHUB_REPO_BASE}/${exampleEndPoints[agent]}/${selectedFolder}/${selectedFile}`
        ...
        <div className="flex h-full w-full max-lg:h-fit max-md:flex-col">
          ...
          <div className="fade-bottom relative flex w-full items-center max-lg:h-fit max-lg:flex-col">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <Image width={443} className="w-full object-contain" src={flows[agent]} alt="Automation Flow" />
              </motion.div>
            </AnimatePresence>
          </div>
        </div>
