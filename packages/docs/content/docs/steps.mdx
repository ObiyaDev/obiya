...
## Anatomy of a Step
Everything in a Motia app is a Step. You can find all the steps of a project under the **_steps_** directory. Each step ends with a `.step` followed by the language extension. So if there's a typescript step, it'll be `step-name.step.ts`. For a Python step, it'll be `step-name.step.py` and so on.
```
my-motia-app/
├── steps/
│   ├── fetch-article.step.ts              # Step to fetch latest article
│   ├── process-article.step.ts          # Step to process article content
│   ├── generate-tweets.step.ts          # Step to generate tweets using LLM
│   └── retrieve-tweets.step.ts            # Step to retrieve stored tweets
```
