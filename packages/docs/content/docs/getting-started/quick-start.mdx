...
<Tab value="Automated">
  <Tabs items={['Using `npx`', 'Using Global CLI']}>
    <Tab value="Using `npx`">
      **Option 1: Using `npx` (No Global Installation Required - Recommended for Quick Start)**

      This is the simplest way to get started, as it doesn't require installing Motia CLI globally. Use `npx` to run the project creation command directly:
      ...
      *   **`npx motia create -t default -n <your-project-name>`**: This command will:
          *   Download and run the Motia CLI project creation tool (using `npx`).
    ...
    <Tab value="Using Global CLI (Optional)">
      **Option 2: Using Globally Installed Motia CLI (Optional)**

      Alternatively, you can install the Motia CLI globally first. This allows you to use the `motia` command directly without `npx`.
