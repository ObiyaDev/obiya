...
description: "Master Motia's comprehensive logging system for debugging workflows and monitoring production applications."
---
...
3. When writing logs, follow consistent patterns for log messages:
...
The `dev` command comes with some [additional flags](/docs/foundations/flows-and-workbench#getting-started-with-motia-workbench) enabling you to use a different port to run the server on, among other things.
</Callout>
</Step>
...
## Next Steps
Motia's logging system transforms debugging from reactive firefighting into proactive system understanding. 

When issues arise in production _(and we all know they will)_, well-structured logs become your primary tool for rapid diagnosis and resolution.
