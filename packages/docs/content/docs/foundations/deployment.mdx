...
- **Retry Capability**: Recover from partial failures by retrying specific phases
...
Once you creat your API key on Motia Cloud, make sure to store it safely. For security reasons, API keys are only revealed once, when you actually create them!
</Callout>
Now, let's take a look the the `deploy` command in detail!
## CLI Reference

```bash
motia deploy [options]
```
...
```bash
motia deploy --environment staging --version 1.2.3-rc.1 --api-key your-api-key-here
```
...
After each deployment, Motia generates two detailed report files in your project directory:

1. `motia.deployments.json`

A comprehensive technical log containing detailed information about every deployment attempt:

```json
{
  "deployments": [
    {
      "bundlePath": "./steps/process-order.zip",
      "deploymentId": "dep_abc123xyz",
      "stepType": "handler",
      "stepName": "ProcessOrder",
      "stepPath": "steps/process-order.ts",
...
A human-readable summary organized by workflows for quick status overview:
...
Make sure you're following all the [deployment best practices.](/docs/foundations/happy-practices#best-practices-of-deploying-a-motia-app)
</Callout>
<Cards>
  <Card
    title="🛠️ Motia CLI"
