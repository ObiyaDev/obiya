...
| 🧰 Built-In Tooling      | Fully integrated with Workbench and [React](/docs/streams#consuming-streams) and browser SDKs. No extra setup needed.           |
...
## Anatomy of a Stream
Streams, simply contain a Stream Config (`StreamConfig`). This is where you define the Stream and it:
...
<Callout type="info" title="Why Streams Are Declarative">
Motia Streams are **declarative:** you define the contract, and Motia handles everything else.

What you define:
- `name`: The stream's unique identifier
- `schema`: What kind of data the stream holds
- `storage`: Where the data is stored

What Motia gives you:
- ✅ **Data validation** out of the box
- 🗃️ **Storage management** with your preferred backend
...
Motia solves this by giving real-time results in the [Workbench](/docs/flows-and-workbench#motia-workbench) where you can test streams end-to-end.
...
Now, simply [open the Workbench](/docs/flows-and-workbench#getting-started-with-motia-workbench), and navigate to http://localhost:3000/endpoints and you'll see your API endpoint. Open the endpoint and click on the `Test` button and you'll see the results automatically getting streamed from the server, in _real-time!_
