...
> Steps have two parts: Config and Handler. Read [anatomy of steps](/docs/foundations/steps#anatomy-of-a-step) to know more.
...
- `input`: parsed and validated input. More on input [here](/docs/foundations/steps#input-data).
- `{ emit, traceId, state, stream, logger }`: Context Object. More on it [here](/docs/foundations/steps#context-object).
...
- You define the `emits` behavior in the step’s `config` block.
...
> You can read more about Emit and Subscribe in [the two behaviours of a step](/docs/foundations/steps#the-two-behaviours-of-a-step).
...
| Feature | Description |
|---------|-------------|
| 🔄 No Polling Needed | Streams push updates automatically |
| 🧰 Built-In Tooling | Fully integrated with Workbench and React/browser SDKs |
| 🔒 Type-Safe Updates | Schema-defined streams ensure type safety |
| 👀 Live Monitoring | Real-time data flow visible in dev environment |
...
| npx motia build | Compile all steps and generate deployment-ready output. | _None_<br/><br/>**Performs the following:**<br/>• Compiles Node.js and Python steps<br/>• Bundles each step into a `.zip` file<br/>• Generates `motia.steps.json` in `dist/`<br/>• Organizes build output in `dist/` |
| npx motia cloud deploy [options] | Deploy the built project to Motia’s deployment service. | `-k, --api-key <key>` (required): API key for authentication<br/>`-e, --environment <env>`: Deployment environment (default: `dev`)<br/>`-v, --version-name <version>`: Version label (default: `latest`)<br/><br/>**Example:**<br/>`npx motia cloud deploy --api-key your-api-key --environment production --version-name 1.0.0`<br/><br/>**This command:**<br/>• Uploads zipped steps with path info<br/>• Uploads `motia.steps.json`<br/>• Starts server-side deployment<br/>• Writes logs to `dist/motia.deployments.json`<br/>• Summarizes results in `dist/motia.deployments.summary.json` |
