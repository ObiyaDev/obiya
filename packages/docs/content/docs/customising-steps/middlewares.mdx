...
description: "Enhance step behavior by injecting reusable logic with middlewares."
---


In API steps, middlewares let you intercept and process incoming requests before handing them off to the associated Step Handlers. 
...
<Callout title="Steps" type="warn">
 Currently, middlewares are only supported by API steps.
</Callout>
...
Middlewares transform your API steps from scattered, repetitive code into clean, composable pipelines.  

They're the difference between writing the same authentication logic fifty times and writing it once, properly. 

Use them to build APIs that are not just functional, but maintainable and scalable.