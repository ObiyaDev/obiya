...
export const handleAxiosError = (error: AxiosError, operation: string): Error => {
    ...
    if (statusCode === 401 || statusCode === 403) {
        return new Error(`Authentication error (${statusCode}): Invalid API key or insufficient permissions. ${responseData}`);
    } else if (statusCode === 404) {
        return new Error(`Not found error (${statusCode}): The API endpoint was not found. ${responseData}`);
    } else if (statusCode === 413) {
        return new Error(`Payload too large (${statusCode}): The ${operation} exceeds the maximum allowed size. ${responseData}`);
