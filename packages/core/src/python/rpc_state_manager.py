...
class RpcStateManager:
    def __init__(self, sender: RpcSender):
        self.sender = sender
    ...
    async def set(self, trace_id: str, key: str, value: Any) -> asyncio.Future[None]:
        return await self.sender.send('state.set', {'traceId': trace_id, 'key': key, 'value': value})

    async def delete(self, trace_id: str, key: str) -> asyncio.Future[None]:
        return await self.sender.send('state.delete', {'traceId': trace_id, 'key': key})

    async def clear(self, trace_id: str) -> asyncio.Future[None]:
        return await self.sender.send('state.clear', {'traceId': trace_id})
