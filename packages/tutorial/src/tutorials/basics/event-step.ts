...
export const eventSteps: TutorialStep[] = [
  ...
  {
    elementXpath: `(//span[contains(text(), 'input')])[1]/..`,
    segmentId,
    ...
    runScriptBeforeNext: () => {
      if (monaco) {
        monaco.editor.getEditors()[0].revealLine(19)
      }
    },
    waitForSelector: '//span[contains(text(), "handler")]',
  },
  {
  ...
  },
  {
    ...
    runScriptBeforePrev: () => {
      if (monaco) {
        monaco.editor.getEditors()[0].revealLine(8)
      }
    },
  },
  {
    elementXpath: `(//span[contains(text(), 'state')])[2]/..`,
    segmentId,
